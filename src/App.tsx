...
import {DEFAULT_SUPPORT,validateSupport} from './game/support';
...
import type {Campaign,Proposal,Role,Upgrade,SupportTactics} from './game/types';
...
const ROLE_LABEL:Record<Role,string>={guard:'盾卫',archer:'射手',assassin:'刺客',support:'司祭'};
...
function Portrait({role,large=false}:{role:Role;large?:boolean}){
 return <div aria-hidden="true" className={`portrait ${role} ${large?'large':''}`} style={{backgroundImage:`url(${portrait(role)})`}}/>;
}
...
export function App(){
 ...
 function changeSupport(value:SupportTactics){
  if(!ready||busy)return;
  const c=structuredClone(campaign);c.heroes.support.strategy.support=validateSupport(value);
  setCampaign(c);setBattle(createBattle(c));resetProposal();
 }
 ...
 function accept(){
  ...
  try{if(proposal.kind==='upgrade'){claimReward(c,proposal);setNotice(`${HEROES[proposal.role as Role].name}获得「${UPGRADES[proposal.upgrade!].name}」`);}
   else{if(!ready)throw new Error('请在备战阶段修改战术。');for(const r of proposal.role==='team'?ROLES:[proposal.role]){
    const {support,...base}=proposal.strategy!;
    c.heroes[r].strategy={...base,...(r==='support'?{support:{...(support??c.heroes[r].strategy.support??DEFAULT_SUPPORT)}}:{})};
   }setBattle(createBattle(c));setNotice('战术已写入。下一场战斗将按新规则执行。');}
 ...
 const suggestions=reward?['学会躲避重击','成功闪避后强化反击','提高攻击力','增加最大生命']:selected==='support'?['生命低于40%再治疗','提前治疗，祝福射手','祝福刺客，虚弱留给重击','交战时释放虚弱']:['优先攻击后排','保护射手','等待前排接敌后行动','生命低时后撤'];
 ...
 return <div className="app-shell">
  ...
  <main>
   <section className="intro"><div><div className="eyebrow"><span/> LANGUAGE INTO TACTICS</div><h1>每一句指令，<em>都是新的可能。</em></h1><p>布下你的阵容，教会他们战斗。让想法在战场上发生。</p></div><div className="run-tag"><span>当前旅程</span><strong>遗迹试炼</strong><small>4 名同伴 · 4 场挑战</small></div></section>
   ...
   <div className="workspace">
    ...
    <aside className={`command-panel ${reward?'reward-mode':''}`}>
     ...
     <div className="selected-hero"><Portrait role={selected}/><div><span>{scope==='team'&&!reward?'指令对象 · 全体同伴':`指令对象 · ${ROLE_LABEL[selected]}`}</span><h3>{scope==='team'&&!reward?'四人小队':hero.name}<small>{scope==='team'&&!reward?'共同的目标':hero.title}</small></h3></div></div>
     ...
     {selected==="support"&&<SupportPanel unit={battle.units.find(u=>u.id==="support")!} tactics={campaign.heroes.support.strategy.support} editable={ready&&!busy} running={running} onChange={changeSupport}/>}
