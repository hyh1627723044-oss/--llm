...
import { beginSupportSkill, resolveSupportSkill, SUPPORT_SKILLS } from './support';
...
export function createBattle(c:Campaign):Battle {
 ...
 return {stage:c.stage,time:0,phase:'ready',units,events:[],effects:[],damage:{},damageTaken:{},healing:{},dodges:{}};
}
...
function hit(b:Battle,u:Unit,t:Unit,heavy=false){
 ...
 const damage=Math.min(t.hp,Math.round(u.damage*(heavy?2.1:1)*(counter?1.8:1)*Math.max(u.rallyUntil>b.time?1.25:1,(u.empowerUntil??0)>b.time?1+SUPPORT_SKILLS.empower.amount:1)*((u.weakenUntil??0)>b.time?1-SUPPORT_SKILLS.weaken.amount:1)));
 ...
 if(t.hp===0){t.intent='已倒下';t.attack=undefined;t.skillCast=undefined;emit(b,'kill',u.id,`${t.name}倒下了`);}
}
export function stepBattle(b:Battle,dt:number){
 ...
 for(const u of alive){
  ...
  if(u.skillCooldowns)for(const skill of ['heal','empower','weaken'] as const)u.skillCooldowns[skill]=Math.max(0,u.skillCooldowns[skill]-dt);
  ...
  if(threat?.attack&&u.strategy.dodge&&u.upgrades.includes('dodge')&&u.dodgeCooldown<=0){
   ...
   u.immune=.8;u.dodgeCooldown=4;u.counterUntil=b.time+3;u.attack=undefined;u.skillCast=undefined;u.intent='闪避重击';
 ...
 for(const u of alive){
  if(u.hp<=0)continue;
  if(resolveSupportSkill(b,u,dt))continue;
  if(u.attack){
   ...
   if(u.attack.remaining<=0){const attack=u.attack;u.attack=undefined;
    ...
    else {const t=b.units.find(v=>v.id===attack.target&&v.hp>0);if(t&&distance(u,t)<=u.range+35){hit(b,u,t);b.effects.push({kind:u.role==='archer'||u.role==='support'?'arrow':'slash',x:u.x,y:u.y-15,tx:t.x,ty:t.y-15,life:.22,maxLife:.22,color:u.team==='ally'?'#d4c38a':'#d7807d'});}}
   }continue;
  }
  ...
  if(beginSupportSkill(b,u))continue;
  const gap=distance(u,t);
  if((u.role==='archer'||u.role==='support')&&gap<105&&u.cooldown>0){u.intent='保持射程';move(u,u.x+(u.x-t.x),u.y+(u.y-t.y),u.speed*dt);}
