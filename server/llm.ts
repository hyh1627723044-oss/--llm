import { SUPPORT_SKILLS } from '../src/game/support';
...
输入 kind 为 strategy 或 upgrade，role 为 guard/archer/assassin/support/team。输出必须原样保持 kind 和 role。
...
target 表示最近/远程后排/最低生命；dodge 是有闪避能力时躲避重击，retreat 是生命低于30%后撤，wait 是等待前排接敌（最多7秒），protect 是优先拦截靠近己方射手的敌人。每次输出完整新策略，不能生成代码。support 是司祭，初始拥有治疗、攻击祝福、虚弱三个技能。仅 role=support 或 team 时可附带 strategy.support:{healAt:0.4|0.7|0.9,buffTarget:"strongest"|"archer"|"assassin",weakenWhen:"heavy"|"engaged"}。healAt 是治疗生命比例阈值；buffTarget 是祝福优先对象；weakenWhen 表示留给重击或交战即用。未涉及辅助时省略 support，保留现有技能策略。技能优先级为治疗、虚弱、祝福。技能定义：${JSON.stringify(SUPPORT_SKILLS)}。其中 amount 对治疗表示生命点数，对祝福/虚弱表示攻击增减比例；所有时间单位为秒。同类效果不叠加，祝福与协同追击取最高值。数值不可修改，不能授予其他角色这些技能；不能同时吟唱，不能复活，不能生成新状态。
